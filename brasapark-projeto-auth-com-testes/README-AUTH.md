...
Use `requests/requests.http` ou `requests/auth-tests.http` no VS Code com a extensao REST Client para testar cadastro, login, erro de login, rota protegida sem token e rota protegida com token.

Os arquivos usam requisicoes nomeadas com `# @name` para capturar automaticamente o token retornado no login:
...
Depois o token e guardado em uma variavel:

```http
@token = {{loginValido.response.body.$.token}}
```

Nas requisicoes protegidas, o token e enviado no header:
