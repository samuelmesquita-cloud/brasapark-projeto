...
async function renderAuthState() {
  ...
  try {
    ...
    avatarPath = data.image?.caminho || "";
  } catch {
    avatarPath = "";
  }

  area.innerHTML = `
    ${avatarPath ? `<img class="nav-avatar" src="${avatarPath}" alt="Avatar de ${user?.nome || "usuario"}">` : ""}
    <span class="nav-user">Ola, ${user?.nome || "usuario"}</span>
    <a class="nav-link-custom" href="./profile.html">Meu perfil</a>
